        ...
        <div className="bg-white p-8 md:p-10 rounded-[3rem] shadow-2xl shadow-[#ff69b4]/5 border" style={{ borderColor: '#F5E6EA' }}>
          ...
          <div className="flex bg-rose-50/50 p-1 rounded-2xl mb-5 border" style={{ borderColor: '#F5E6EA' }}>
            {['login', 'signup'].map((mode) => (
              <button
                ...
                className={`flex-1 py-3 text-[9px] font-black uppercase tracking-widest rounded-xl transition-all ${authMode === mode ? 'bg-white text-[#2D1520] shadow-sm border' : 'text-[#C4A0AC]'}`}
              ...
              {roles.map((r) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 flex flex-col items-center gap-1.5 py-3 rounded-xl transition-all relative ${isActive ? 'text-[#2D1520]' : 'text-[#C4A0AC]'}`}
                  >
                    {isActive && (
                      <motion.div
                        layoutId="activeRole"
          ...
          {authMode === 'login' ? (
            ...
            <form onSubmit={handleSignup} className="space-y-3">
              ...
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} type="submit" className="w-full py-4 text-[11px] font-black uppercase tracking-[0.3em] text-white rounded-[2rem] shadow-2xl transition-all flex items-center justify-center gap-3" style={{ background: '#2D1520' }}>
