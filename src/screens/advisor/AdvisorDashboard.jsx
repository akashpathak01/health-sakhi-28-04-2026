   ...
   return (
      <div className="space-y-8 animate-in fade-in duration-500">
         ...
         <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
               <h1 className="text-3xl font-black tracking-tight text-[#2D1520]">Namaste, Dr. Sakhi.</h1>
            ...
            <div className="flex bg-white p-1 rounded-2xl border border-[#F5E6EA] shadow-sm self-start">
               {['Today', 'This Week'].map(tab => (
                  <button 
                    ...
                    className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-[#2D1520] text-white shadow-xl shadow-[#2D1520]/20' : 'text-[#C4A0AC] hover:text-[#2D1520]'}`}
                  >
                    {tab}
                  </button>
               ))}
         ...
         <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
            {stats.map((stat, i) => (
               ...
               >
                  ...
                  <h3 className="text-2xl font-black text-[#2D1520] tracking-tight">{stat.value}</h3>
               </motion.div>
            ))}
         </div>
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="lg:col-span-8 bg-white rounded-[3rem] shadow-sm border border-[#F5E6EA] overflow-hidden">
               <div className="p-8 flex items-center justify-between border-b border-[#F5E6EA]">
                  <div>
                     <h2 className="text-xl font-black tracking-tight text-[#2D1520]">Upcoming Sessions</h2>
                     ...
                     <div key={i} className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 hover:bg-rose-50/20 transition-all group">
                        <div className="flex items-center gap-5">
                           ...
                           <div className="space-y-1">
                              <p className="text-base font-black text-[#2D1520]">{apt.name}</p>
                        ...
                        <div className="flex items-center gap-6 self-end md:self-auto">
                           <div className="text-right">
                              ...
                              <p className="text-sm font-black text-[#2D1520]">{apt.time}</p>
                           </div>
                           <button 
            ...
            <div className="lg:col-span-4 space-y-8">
               <div className="bg-[#2D1520] p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden h-full">
         ...
         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div onClick={() => navigate('/advisor/chat')} className="bg-white p-8 rounded-[3rem] shadow-sm border border-[#F5E6EA] flex items-center justify-between group cursor-pointer hover:shadow-md transition-all">
               <div className="flex items-center gap-6">
                  ...
                  <div>
                     <h3 className="text-xl font-black text-[#2D1520]">Communication Hub</h3>
            ...
            <div onClick={() => navigate('/advisor/users')} className="bg-white p-8 rounded-[3rem] shadow-sm border border-[#F5E6EA] flex items-center justify-between group cursor-pointer hover:shadow-md transition-all">
               <div className="flex items-center gap-6">
                  <div className="w-14 h-14 rounded-2xl bg-rose-50 flex items-center justify-center text-[#2D1520] shadow-inner group-hover:scale-110 transition-transform">
                     <Users size={24} />
                  </div>
                  <div>
                     <h3 className="text-xl font-black text-[#2D1520]">Patient Registry</h3>
                     <p className="text-xs font-medium text-[#C4A0AC]">Review clinical health cards</p>
                  </div>
               </div>
               <ArrowRight size={20} className="text-[#F5E6EA] group-hover:text-[#2D1520] group-hover:translate-x-2 transition-all" />
            </div>
         </div>
         ...
         <AnimatePresence>
            {activeCall && (
               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-[#2D1520] flex items-center justify-center">
                  <div className="absolute inset-0 bg-gradient-to-br from-[#2D1520] via-black to-[#2D1520] flex items-center justify-center overflow-hidden">
                     ...
                     <div className="text-center space-y-8 relative z-10">
                        <div className="w-48 h-48 rounded-[3.5rem] bg-white/5 border border-white/10 flex items-center justify-center mx-auto relative group">
                           ...
                           <div className="absolute top-2 right-2 w-12 h-12 bg-[#ff69b4] rounded-2xl border-4 border-[#2D1520] flex items-center justify-center">
