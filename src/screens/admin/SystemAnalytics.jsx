...
const AnalyticModal = ({ open, onClose, title, subtitle, icon: Icon, iconBg, children }) => {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 bg-[#2D1520]/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white rounded-t-[2.5rem] sm:rounded-[3rem] w-full sm:max-w-2xl shadow-2xl border animate-in slide-in-from-bottom sm:zoom-in-95 duration-500 max-h-[92vh] flex flex-col overflow-hidden relative" style={{ borderColor: '#F5E6EA' }}>
        <div className="p-4 sm:p-6 border-b flex items-center justify-between flex-shrink-0 bg-rose-50/30" style={{ borderColor: '#F5E6EA' }}>
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg sm:text-2xl font-bold tracking-tight leading-none" style={{ color: '#2D1520' }}>{title}</h2>
  ...
  return (
    <div className="space-y-10 font-sans pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 px-1">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight leading-none mb-2" style={{ color: '#2D1520' }}>Business Intelligence</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
           <button onClick={() => setActiveModal('range')} className="px-3 sm:px-4 py-2 bg-white border text-[8px] font-black rounded-2xl tracking-tight flex items-center justify-center gap-2 hover:bg-rose-50/50 transition-all shadow-sm" style={{ borderColor: '#F5E6EA', color: '#2D1520' }}>
             <Clock size={14} /> <span className="truncate">Custom Range</span>
           </button>
           <button onClick={() => { setActiveModal('audit'); triggerAudit(); }} className="px-3 sm:px-4 py-2 text-white font-black rounded-2xl text-[8px] uppercase tracking-widest flex items-center justify-center gap-2 shadow-xl transition-all hover:brightness-110 active:scale-95" style={{ background: '#2D1520' }}>
             Audit Analytics
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'CAC (Avg)', val: '₹420', growth: '-5%', up: true, icon: Zap, color: '#2D1520', bg: '#FDEEF1' },
          ...
          <div key={i} className={`p-4 rounded-[1.75rem] border shadow-sm hover:shadow-md transition-all relative overflow-hidden group`} style={{ background: m.bg, borderColor: '#F5E6EA' }}>
             ...
             <p className="text-xl font-black tracking-tight relative z-10" style={{ color: '#2D1520' }}>{m.val}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
         ...
         <div className="bg-white p-4 rounded-[2rem] border border-black/5 shadow-sm space-y-6 relative overflow-hidden">
            <div className="absolute top-0 right-0 w-40 h-40 bg-health-green/5 rounded-full blur-3xl -mr-20 -mt-20"></div>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative z-10">
               ...
               <div className="flex gap-3">
                  <div className="flex items-center gap-2">
                     <div className="w-2 h-2 rounded-full bg-health-green shadow-[0_0_8px_rgba(34,197,94,0.4)]"></div>
               ...
               {[40, 60, 45, 90, 65, 80, 50, 70, 85, 95, 75, 100].map((h, i) => (
                  <div key={i} className="flex-1 flex flex-col items-center gap-3 group cursor-pointer h-full justify-end">
                     <div className="w-full flex-1 flex flex-col gap-1 justify-end">
                        <div className="w-full bg-health-green rounded-xl group-hover:brightness-110 transition-all shadow-sm shadow-health-green/10 group-hover:scale-x-110" style={{ height: `${h}%` }}></div>
         ...
         <div className="bg-white p-4 rounded-[2.5rem] border shadow-sm flex flex-col" style={{ borderColor: '#F5E6EA' }}>
            <div className="mb-4">
                <h2 className="text-lg font-bold tracking-tight leading-none" style={{ color: '#2D1520' }}>Revenue Distribution</h2>
            ...
            <div className="space-y-6 flex-1">
               {[
                 ...
                 { label: 'Senior Wellness', val: '18%', color: '#2D1520' },
                 { label: 'General Lifestyle', val: '15%', color: '#F5B4BC' },
               ].map((item, i) => (
                  <div key={i} className="space-y-3 group cursor-pointer">
                     <div className="flex justify-between items-center px-1">
                        <span className="text-[8px] font-bold uppercase tracking-widest group-hover:text-[#2D1520] transition-colors" style={{ color: '#C4A0AC' }}>{item.label}</span>
                        <div className="flex items-center gap-2">
                           <span className="text-base font-black tracking-tight group-hover:scale-110 transition-transform" style={{ color: '#2D1520' }}>{item.val}</span>
            ...
            <div className="mt-5 pt-5 border-t flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4" style={{ borderColor: '#F5E6EA' }}>
               <div className="flex items-center gap-3">
                  ...
                  <div>
                      <p className="text-xs font-bold uppercase tracking-widest mb-1 leading-none" style={{ color: '#2D1520' }}>Meta Intelligence</p>
      ...
      <div className="p-4 border rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-4 relative overflow-hidden" style={{ background: '#FFF7F8', borderColor: '#F5E6EA' }}>
         ...
         <div className="space-y-3 relative z-10">
            ...
            <h3 className="text-lg font-black tracking-tight" style={{ color: '#2D1520' }}>Audit-Ready Financial Data</h3>
         ...
         <button onClick={triggerVault} disabled={isProcessing} className="px-4 py-2 bg-white border-2 text-[8px] font-bold uppercase tracking-[0.3em] rounded-[1.75rem] shadow-xl hover:bg-[#2D1520] hover:text-white transition-all active:scale-95 relative z-10" style={{ color: '#2D1520', borderColor: '#2D1520' }}>
            {isProcessing ? 'Verifying Node...' : 'Open Auditor Vault'}
         </button>
      </div>

      {/* ── Modals ── */}
      <AnalyticModal open={activeModal === 'range'} onClose={() => setActiveModal(null)} title="Temporal Range" subtitle="Select analysis period" icon={Clock} iconBg="bg-[#2D1520]">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {['Last 24 Hours', 'Past 7 Days', 'Current Month', 'Fiscal Year 2024', 'Last 365 Days', 'Lifetime Analysis'].map(range => (
                  <button key={range} onClick={() => setActiveModal(null)} className="p-3 bg-rose-50/20 border rounded-[1.75rem] text-left hover:border-[#ff69b4] transition-all group" style={{ borderColor: '#F5E6EA' }}>
                      ...
                      <p className="font-bold text-sm" style={{ color: '#2D1520' }}>{range}</p>
                  </button>
              ))}
          </div>
      </AnalyticModal>

      <AnalyticModal open={activeModal === 'audit'} onClose={() => setActiveModal(null)} title="Deep System Audit" subtitle="Financial Reconciliation Engine" icon={ShieldCheck} iconBg="bg-[#2D1520]">
              ...
              <div className="space-y-4 animate-pulse">
                  ...
                  <div className="text-center space-y-6 animate-in zoom-in-95 duration-500">
                      ...
                      <div className="space-y-2">
                        <h3 className="text-xl font-bold tracking-tight" style={{ color: '#2D1520' }}>Audit Successful</h3>
                ...
                { time: '14:18:55', event: 'Advisor Payout Ledger Synced', type: 'SUCCESS' },
                  ...
                  <div key={i} className="p-3 bg-rose-50/20 border rounded-xl flex items-center justify-between group hover:bg-white transition-all" style={{ borderColor: '#F5E6EA' }}>
                      <div className="flex items-center gap-3">
                          ...
                          <span className="text-[10px] font-bold tracking-tight" style={{ color: '#2D1520' }}>{log.event}</span>
                      </div>
      ...
      <AnalyticModal open={activeModal === 'vault'} onClose={() => { setActiveModal(null); setVaultAuthorized(false); setPin(''); }} title="Auditor Vault" subtitle={vaultAuthorized ? "Authorized Access Root" : "Restricted Financial Ledgers"} icon={vaultAuthorized ? Shield : Lock} iconBg={vaultAuthorized ? "bg-[#ff69b4]" : "bg-red-500"}>
          {!vaultAuthorized ? (
              <div className="space-y-8 text-center p-4">
                  ...
                  <div className="space-y-2">
                    <h3 className="text-2xl font-bold tracking-tight leading-none mb-3" style={{ color: '#2D1520' }}>Security Protocol Required</h3>
              ...
              <div className="space-y-8 animate-in zoom-in-95 duration-700">
                  <div className="p-6 bg-[#FDEEF1] rounded-[2rem] border flex items-center justify-between" style={{ borderColor: '#F5E6EA' }}>
                      <div>
                          ...
                          <h4 className="text-xl font-bold tracking-tight" style={{ color: '#2D1520' }}>Root Ledger Connected</h4>
                      </div>
