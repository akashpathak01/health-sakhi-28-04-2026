...
const MailModal = ({ user, onClose }) => {
  ...
  return (
    <Modal open={!!user} onClose={onClose} title="Dispatch Email" subtitle={user?.email} icon={Mail} iconBg="bg-[#2D1520]">
      {sent ? (<div className="flex flex-col items-center justify-center py-10 gap-4 animate-in zoom-in-95"><div className="w-20 h-20 rounded-full bg-rose-50 flex items-center justify-center shadow-inner"><CheckCircle size={40} style={{ color: '#ff69b4' }} /></div><p className="text-xl font-black text-slate-800">Email Dispatched!</p></div>) : (
        <form onSubmit={handleSend} className="space-y-5">
          <div className="space-y-2">
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Subject Line</label><input required value={form.subject} onChange={e => setForm({ ...form, subject: e.target.value })} className="w-full h-12 px-4 bg-slate-50 border border-black/5 rounded-xl outline-none focus:border-health-green text-sm font-bold shadow-inner" /></div>
            <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Message Body</label><textarea required value={form.body} onChange={e => setForm({ ...form, body: e.target.value })} className="w-full h-32 p-4 bg-slate-50 border border-black/5 rounded-xl outline-none focus:border-health-green text-sm font-bold resize-none shadow-inner" /></div>
          </div>
  ...
  return (
    <div className="space-y-8 font-sans pb-20 animate-in fade-in duration-700">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl sm:text-4xl font-extrabold text-[#2D1520] tracking-tight leading-none mb-2">Member Portal Management</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[{ l: 'Total Sisters', v: '12,482', c: 'text-slate-800' }, { l: 'Premium Tier', v: '2,841', c: 'text-health-green' }, { l: 'Online Now', v: '849', c: 'text-blue-500' }, { l: 'Trials Active', v: '1,202', c: 'text-amber-500' }].map((s, i) => (
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center bg-white p-2 rounded-[2.5rem] border border-black/5 shadow-inner">
        ...
        <div className="flex gap-2 pr-2">
          <button onClick={() => setShowFilter(!showFilter)} className={`px-4 h-10 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all flex items-center gap-2 ${showFilter ? 'bg-health-green text-white' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}><Filter size={16} /> Filters</button>
