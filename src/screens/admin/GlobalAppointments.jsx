  ...
  return (
    <div className="space-y-10 font-sans pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-2xl font-black tracking-tight leading-none mb-2" style={{ color: '#2D1520' }}>Consultation Master-View</h1>
        ...
        <div className="flex bg-white p-1 rounded-2xl border shadow-sm" style={{ borderColor: '#F5E6EA' }}>
           <button onClick={() => setActiveView('Grid')} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-sm ${activeView === 'Grid' ? 'bg-[#2D1520] text-white' : 'bg-transparent text-[#C4A0AC] hover:text-[#2D1520] hover:bg-rose-50'}`}>Real-time Grid</button>
           <button onClick={() => setActiveView('History')} className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all shadow-sm ${activeView === 'History' ? 'bg-[#2D1520] text-white' : 'bg-transparent text-[#C4A0AC] hover:text-[#2D1520] hover:bg-rose-50'}`}>History Log</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4">
               ...
               <div className="flex w-full lg:w-auto gap-2">
                   {['All Mode', 'Live', 'Scheduled'].map(mode => (
                       <button key={mode} onClick={() => setFilterMode(mode)} className={`flex-1 lg:flex-none h-11 px-4 rounded-2xl text-[9px] font-black uppercase tracking-widest transition-all ${filterMode === mode ? 'bg-[#2D1520] text-white shadow-lg' : 'bg-white border text-[#C4A0AC] hover:bg-rose-50'}`} style={{ borderColor: '#F5E6EA' }}>
                           {mode}
                       </button>
                   ))}
               </div>
                        ...
                        {filtered.map((apt) => (
                           <tr key={apt.id} className="hover:bg-slate-50/50 transition-colors group">
                              <td className="px-4 py-4">
                                 <p className="font-extrabold text-[#2D1520] tracking-tight leading-none mb-1">{apt.id}</p>
                              ...
                              <td className="px-4 py-4">
                                 <div className="space-y-1.5">
                                    <div className="flex items-center gap-2">
                                       ...
                                       <span className="font-bold tracking-tight leading-none" style={{ color: '#2D1520' }}>{apt.user}</span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                       <span className="text-[9px] font-bold uppercase tracking-widest w-12" style={{ color: '#C4A0AC' }}>Expert:</span>
                                       <span className="font-bold tracking-tight leading-none flex items-center gap-1.5" style={{ color: '#2D1520' }}><ShieldCheck size={12} style={{ color: '#ff69b4' }}/> {apt.advisor}</span>
                                    </div>
                                 </div>
                              </td>
                              <td className="px-4 py-4">
                              ...
                              <td className="px-4 py-4 text-right">
                                 <button onClick={() => setViewApt(apt)} className="p-2 bg-white border rounded-lg text-slate-300 group-hover:text-[#2D1520] group-hover:border-[#ff69b4] transition-all hover:shadow-sm" style={{ borderColor: '#F5E6EA' }}>
                                    <Eye size={18} />
                                 </button>
                              </td>
                           </tr>
         ...
         <div className="space-y-6 animate-in slide-in-from-right-6 duration-500">
            <h3 className="text-lg font-bold tracking-tight px-2 leading-none" style={{ color: '#2D1520' }}>Operation Center</h3>
            <div className="bg-[#2D1520] text-white rounded-[2rem] p-5 shadow-2xl space-y-6 relative overflow-hidden">
      ...
      <CMSModal open={!!viewApt} title="Consultation Details" icon={Eye} iconBg="bg-[#2D1520]" onClose={() => setViewApt(null)}>
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
              <div className="flex items-center gap-4">
                  <div className="w-16 h-16 rounded-3xl bg-[#2D1520] flex items-center justify-center text-white font-black text-xl shadow-xl">{viewApt?.user.charAt(0)}</div>
                  <div className="space-y-1">
                      <h3 className="text-2xl font-bold tracking-tight leading-none" style={{ color: '#2D1520' }}>{viewApt?.user}</h3>
              ...
              <div className="grid grid-cols-2 gap-3">
                  <div className="p-4 bg-rose-50/20 border rounded-[1.75rem] space-y-1.5" style={{ borderColor: '#F5E6EA' }}>
                       ...
                       <p className="text-sm font-bold flex items-center gap-2" style={{ color: '#2D1520' }}><ShieldCheck size={16} style={{ color: '#ff69b4' }}/> {viewApt?.advisor}</p>
                  </div>
                  <div className="p-4 bg-rose-50/20 border rounded-[1.75rem] space-y-1.5" style={{ borderColor: '#F5E6EA' }}>
                       <p className="text-[9px] font-bold uppercase tracking-widest" style={{ color: '#C4A0AC' }}>Scheduled Slot</p>
                       <p className="text-sm font-bold flex items-center gap-2" style={{ color: '#2D1520' }}><Clock size={16} className="text-blue-500"/> {viewApt?.time}</p>
                  </div>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                  <button onClick={handleJoinSession} disabled={isJoining} className="flex-1 py-3 text-white font-bold text-[9px] uppercase tracking-[0.2em] rounded-2xl hover:brightness-110 transition-all flex items-center justify-center gap-2 shadow-xl shadow-[#2D1520]/10" style={{ background: '#2D1520' }}>
