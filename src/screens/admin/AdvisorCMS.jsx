...
const FormDropdown = ({ label, value, options, onChange }) => {
  ...
  return (
    <div className="relative space-y-2" ref={dropdownRef}>
      ...
      <button type="button" onClick={() => setIsOpen(!isOpen)} className={`w-full h-10 px-4 flex items-center justify-between bg-slate-50 border rounded-xl transition-all font-bold text-[9px] ${isOpen ? 'border-health-green ring-4 ring-health-green/5' : 'border-black/5 hover:border-slate-200'}`}>
      ...
      {isOpen && (
        <div className="absolute top-[78px] left-0 w-full bg-white border border-black/5 rounded-xl shadow-xl z-[60] p-1.5 animate-in slide-in-from-top-2 duration-200">
          {options.map((opt) => (
            <button key={opt} type="button" onClick={() => { onChange(opt); setIsOpen(false); }} className={`w-full text-left px-4 py-2 rounded-lg text-[9px] font-bold transition-all ${value === opt ? 'bg-health-green text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
              {opt}
            </button>
          ))}
        </div>
    ...
    <div className="space-y-8 font-sans pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 text-sm font-medium">Verify credentials and manage schedules for <span className="text-health-green font-black">{advisors.length} advisors.</span></p>
        </div>
      ...
      <div className="bg-white rounded-[2.5rem] border border-black/5 shadow-sm overflow-hidden">
        <div className="p-4 border-b border-black/5 flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-slate-50/20">
           ...
           <div className="relative w-full lg:max-w-sm sm:mr-4 px-1">
              ...
              <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="Find advisor by name or specialty..." className="w-full h-10 pl-11 pr-5 bg-white border border-black/5 rounded-2xl text-xs font-bold outline-none focus:border-health-green transition-all shadow-sm" />
           </div>
        </div>
                 ...
                 <tr className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-4 py-4">
                       <span className={`inline-flex items-center gap-1 text-[8px] font-black uppercase tracking-widest ${
                          adv.status === 'Approved' ? 'text-health-green' : 
                          adv.status === 'Review' ? 'text-blue-500' : 'text-orange-500'
                       }`}>
                          <div className={`w-1.5 h-1.5 rounded-full ${adv.status === 'Approved' ? 'bg-health-green' : 'bg-current animate-pulse'}`}></div>
                          {adv.status}
                       </span>
                    </td>
      ...
      {/* ── CREATE / EDIT MODAL ────────────────────────────────────────────────── */}
      {(isAddOpen || editAdv) && (
        <CMSModal open title={editAdv ? 'Modify Expert Credentials' : 'Recruit New Advisor'} icon={UserPlus} iconBg={editAdv ? 'bg-health-green' : 'bg-slate-900'} onClose={() => { setIsAddOpen(false); setEditAdv(null); resetForm(); }}>
           <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-2 gap-3">
                 <div className="space-y-1 col-span-2"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Full Name</label><input required value={formData.name} onChange={e=>setFormData({...formData, name: e.target.value})} className="w-full h-10 px-3 bg-slate-50 border border-black/5 rounded-xl outline-none focus:border-health-green text-sm font-bold shadow-inner" placeholder="Dr. Sarah Jones" /></div>
         ...
         <div className="space-y-8 animate-in fade-in duration-500">
            <div className="flex items-center gap-6">
                ...
                <div className="space-y-1.5">
                    ...
                    <p className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none"><ShieldCheck size={14} className="text-health-green"/> {viewAdv?.specialty}</p>
