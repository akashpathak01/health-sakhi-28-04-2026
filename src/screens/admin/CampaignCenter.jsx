...
const CMSModal = ({ open, onClose, title, icon: Icon, iconBg, children }) => {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-[#2D1520]/60 backdrop-blur-md animate-in fade-in duration-200">
      <style>{`
      ...
      <div className="bg-white rounded-t-[2.5rem] sm:rounded-[2.5rem] w-full sm:max-w-2xl shadow-2xl border animate-in slide-in-from-bottom sm:zoom-in-95 duration-500 max-h-[92vh] flex flex-col overflow-hidden relative" style={{ borderColor: '#F5E6EA' }}>
        <div className="p-6 sm:p-8 border-b flex items-center justify-between flex-shrink-0 bg-rose-50/30" style={{ borderColor: '#F5E6EA' }}>
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg sm:text-2xl font-black tracking-tight leading-none" style={{ color: '#2D1520' }}>{title}</h2>
          </div>
  ...
  return (
    <div className="space-y-10 font-sans pb-20 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-5">
        <div>
          <h1 className="text-2xl font-black tracking-tight leading-none mb-2" style={{ color: '#2D1520' }}>Campaign Center</h1>
        ...
        <button
          ...
          className="px-4 py-2 bg-[#2D1520] text-white font-black rounded-2xl text-[9px] uppercase tracking-[0.2em] flex items-center justify-center gap-2 shadow-2xl hover:scale-105 active:scale-95 transition-all shadow-[#2D1520]/20"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="p-3 sm:p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-5 bg-rose-50/20 px-2 lg:px-5">
              <div className="flex flex-wrap gap-2">
                {['All', 'Active', 'Scheduled', 'Completed'].map(t => (
                  <button key={t} onClick={() => setActiveTab(t)} className={`flex-1 sm:flex-none px-3 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${activeTab === t ? 'bg-[#2D1520] text-white shadow-lg' : 'bg-white text-[#C4A0AC] border border-[#F5E6EA] hover:bg-rose-50'}`}>{t}</button>
                ))}
              </div>
                  ...
                  {filtered.map((cmp) => (
                    <tr key={cmp.id} className="hover:bg-rose-50/10 transition-colors group">
                      <td className="px-4 py-4">
                        <p className="font-extrabold text-[#2D1520] tracking-tight leading-none mb-1">{cmp.name}</p>
                      ...
                      <td className="px-4 py-4">
                        <div className="space-y-1">
                          <p className="text-[9px] font-bold text-[#2D1520] tracking-tight">{cmp.sent} <span className="text-[9px] font-bold text-[#C4A0AC] uppercase">Reached</span></p>
                      ...
                      <td className="px-4 py-4 text-right relative">
                        <button onClick={() => setActiveMenu(activeMenu === cmp.id ? null : cmp.id)} className={`p-2 bg-white border rounded-lg hover:shadow-sm transition-all shadow-sm ${activeMenu === cmp.id ? 'border-[#ff69b4] ring-4 ring-[#ff69b4]/5 text-[#ff69b4]' : 'text-[#C4A0AC] border-[#F5E6EA] hover:text-[#2D1520] hover:border-[#C4A0AC]'}`}>
        ...
        <div className="space-y-5 animate-in slide-in-from-right-6 duration-500">
          <h2 className="text-lg font-bold tracking-tight px-2 leading-none" style={{ color: '#2D1520' }}>Flash Broadcast</h2>
          <div className="bg-white rounded-[2.5rem] p-6 border shadow-2xl space-y-5 relative overflow-hidden" style={{ borderColor: '#F5E6EA' }}>
            ...
            <button onClick={handleQuickSend} disabled={isSendingFlash} className="w-full py-3 text-white rounded-[2rem] font-bold text-[9px] uppercase tracking-[0.2em] shadow-2xl hover:brightness-110 hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-2 group shadow-[#2D1520]/20" style={{ background: '#2D1520' }}>
      ...
      <CMSModal open={isModalOpen} onClose={() => { setIsModalOpen(false); setFormData({ id: null, name: '', audience: 'All App Users', type: 'Email + Push', body: '', date: '', time: '' }); }} title={formData.id ? "Edit Campaign Meta" : "New Broadcast Campaign"} icon={formData.id ? Edit2 : Bell} iconBg="bg-[#2D1520]">
        <form className="space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="pt-6 border-t flex flex-col sm:flex-row justify-end gap-3 mt-4" style={{ borderColor: '#F5E6EA' }}>
            ...
            <button type="submit" className="px-6 py-2 text-white font-bold text-[9px] uppercase tracking-[0.2em] rounded-2xl hover:brightness-110 active:scale-95 transition-all shadow-xl shadow-[#2D1520]/20" style={{ background: '#2D1520' }}>{formData.id ? "Update Campaign" : "Schedule Broadcast"}</button>
          </div>
        </form>
      </CMSModal>
        ...
        <div className="space-y-6 animate-in zoom-in-95 duration-500">
          <div className="p-6 bg-rose-50/30 rounded-[2.5rem] border flex items-center justify-between relative overflow-hidden" style={{ borderColor: '#F5E6EA' }}>
            <div className="relative z-10">
              ...
              <h4 className="text-2xl font-bold tracking-tight leading-none" style={{ color: '#2D1520' }}>Ready to Re-dispatch</h4>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-4 bg-rose-50/20 border rounded-3xl space-y-3" style={{ borderColor: '#F5E6EA' }}>
              ...
              <p className="text-sm font-bold" style={{ color: '#2D1520' }}>{resendCmp?.audience}</p>
            </div>
            <div className="p-6 bg-rose-50/20 border rounded-3xl space-y-3" style={{ borderColor: '#F5E6EA' }}>
              ...
              <p className="text-sm font-bold" style={{ color: '#2D1520' }}>~14,200 High</p>
            </div>
          </div>
          ...
          <div className="flex gap-4">
            <button onClick={handleResend} disabled={isSendingFlash} className="flex-1 py-3 text-white font-bold text-[9px] uppercase tracking-widest rounded-2xl hover:brightness-110 transition-all flex items-center justify-center gap-2 shadow-xl shadow-[#2D1520]/20" style={{ background: '#2D1520' }}>
