...
const StatCard = ({ title, value, change, icon: Icon, color, onClick }) => (
  ...
  >
    ...
    <p className="text-2xl font-black tracking-tight" style={{ color: '#2D1520' }}>{value}</p>
  </motion.div>
);
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500 pb-20">
      ...
      <section className="flex flex-col md:flex-row md:items-center justify-between gap-6">
         <div>
            <h1 className="text-3xl font-black tracking-tight text-[#2D1520]">Admin Command Center</h1>
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#C4A0AC] mt-1">Managing the HealthSakhi Ecosystem</p>
         </div>
         <div className="flex gap-4">
            <button onClick={() => navigate('/admin/content')} className="px-6 py-4 bg-[#2D1520] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center gap-2 hover:bg-[#ff69b4] transition-all">
               <PlusCircle size={16} /> New Content
            </button>
            <button onClick={() => navigate('/admin/advisors')} className="px-6 py-4 bg-white border border-[#F5E6EA] text-[#2D1520] rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-sm flex items-center gap-2 hover:border-[#ff69b4] transition-all">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard onClick={() => navigate('/admin/users')} title="Total Users" value="12,482" change="+12.5%" icon={Users} color="#2D1520" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-white p-10 rounded-[3rem] border border-[#F5E6EA] shadow-sm relative overflow-hidden">
           <div className="flex items-center justify-between mb-12 relative z-10">
              <div>
                <h3 className="text-2xl font-black tracking-tight text-[#2D1520]">User Growth</h3>
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center h-full group relative">
                       <div className="w-full h-full flex items-end">
                          ...
                          >
                             {/* Tooltip on Hover */}
                             <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity bg-[#2D1520] text-white text-[8px] font-bold px-1.5 py-0.5 rounded pointer-events-none whitespace-nowrap z-20">
        ...
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-[#2D1520] p-8 rounded-[3rem] text-white shadow-2xl relative overflow-hidden">
           ...
           <div className="bg-white p-8 rounded-[3rem] border border-[#F5E6EA] shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                 ...
                 <h3 className="text-[10px] font-black uppercase tracking-widest text-[#2D1520]">System Health</h3>
              </div>
              <div className="space-y-4">
                 ...
                 <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-black text-[#2D1520]">65%</span>
                 </div>
              </div>
           </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
         <div className="lg:col-span-7 bg-white p-8 rounded-[3rem] border border-[#F5E6EA] shadow-sm">
            <h3 className="text-xl font-black tracking-tight text-[#2D1520] mb-8">Engagement Nodes</h3>
            <div className="space-y-5">
               {[
                 ...
                 { label: 'Sakhi Chatbot Usage', type: 'System', views: '1.2k', color: '#2D1520', pct: 45 }
               ].map((item, i) => (
                 <div key={i} className="p-5 bg-rose-50/20 rounded-2xl border border-[#F5E6EA] flex items-center gap-6 group hover:shadow-md transition-all">
                    ...
                    <div className="flex-1">
                       <div className="flex justify-between items-center mb-2">
                          <p className="text-sm font-black text-[#2D1520]">{item.label}</p>
         ...
         <div className="lg:col-span-5 bg-white p-8 rounded-[3rem] border border-[#F5E6EA] shadow-sm flex flex-col">
            <div className="flex items-center gap-3 mb-8">
               ...
               <h3 className="text-xl font-black tracking-tight text-[#2D1520]">Sakhi Pulse Monitor</h3>
            </div>
               ...
               {[
                 ...
                 <div key={i} className="flex items-center justify-between p-4 bg-rose-50/10 rounded-2xl">
                    <div className="flex items-center gap-3">
                       ...
                       <span className="text-xs font-bold text-[#2D1520]">{item.q}</span>
                    </div>
