...
const CAT_COLORS = {
  ...
  'Mental Health': 'bg-blue-50 text-blue-600 border-blue-100',
  ...
  return (
    <div className="relative space-y-2" ref={dropdownRef}>
      ...
      <button 
        ...
        className={`w-full h-12 px-4 flex items-center justify-between bg-slate-50 border rounded-xl transition-all font-bold text-sm ${isOpen ? 'border-health-green ring-2 ring-health-green/5' : 'border-black/5 hover:border-slate-200'}`}
      >
        ...
        <div className="absolute top-[74px] left-0 w-full bg-white border border-black/5 rounded-xl shadow-xl z-50 p-1.5 animate-in slide-in-from-top-2 duration-200 overflow-hidden">
          {options.map((opt) => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-lg text-sm font-bold transition-all ${value === opt ? 'bg-health-green text-white' : 'text-slate-600 hover:bg-slate-50'}`}
            >
              {opt}
            </button>
          ))}
...
const StyledDropdown = ({ label, value, options, onChange, icon: Icon }) => {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button onClick={() => setIsOpen(!isOpen)} className={`h-10 px-4 rounded-2xl border transition-all flex items-center gap-3 font-black text-[9px] uppercase tracking-widest min-w-[150px] justify-between ${isOpen ? 'bg-white border-health-green ring-4 ring-health-green/5' : 'bg-white border-black/5 hover:border-slate-200'}`}>
        <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-lg ${isOpen ? 'bg-health-green text-white' : 'bg-slate-50 text-slate-400'}`}><Icon size={14} /></div>
            <span className={isOpen ? 'text-health-green' : 'text-slate-500'}>{value}</span>
        </div>
        <ChevronDown size={16} className={`transition-transform duration-300 text-slate-300 ${isOpen ? 'rotate-180 text-health-green' : ''}`} />
      </button>
      {isOpen && (
        <div className="absolute top-16 left-0 w-full bg-white rounded-2xl shadow-2xl border border-black/5 p-2 z-30 animate-in zoom-in-95 duration-200">
           ...
           {options.map(opt => (
               <button key={opt} onClick={() => { onChange(opt); setIsOpen(false); }} className={`w-full text-left px-4 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${value === opt ? 'bg-health-green/10 text-health-green' : 'text-slate-500 hover:bg-slate-50'}`}>{opt}</button>
           ))}
        </div>
      )}
    </div>
...
const ContentCMS = () => {
  ...
  const [formData, setFormData] = useState({ title: '', type: 'Video Module', cat: 'Women Health', desc: '' });

  const resetForm = () => {
    setFormData({ title: '', type: 'Video Module', cat: 'Women Health', desc: '' });
  ...
  const handleUpdate = (e) => {
    ...
    if (editItem) {
        setContent(prev => prev.map(item => item.id === editItem.id ? { ...item, ...formData, type: formData.type.includes('Video') ? 'Video' : 'Book', mediaUrl } : item));
        setEditItem(null);
    } else {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
           <div className="flex items-center gap-3 mb-2">
                <div className="w-1.5 h-8 bg-health-green rounded-full shadow-lg shadow-health-green/20"></div>
                <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-800 tracking-tight leading-none">Content Management</h1>
           </div>
          <p className="text-slate-400 font-medium ml-4 tracking-tight leading-none">Upload and oversee your <span className="text-health-green font-black">wellness repository.</span></p>
        </div>
        <button 
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        <div className="flex-1 relative w-full">
          ...
          <input 
            ...
            className="w-full h-10 pl-12 pr-4 rounded-xl border border-black/5 bg-white shadow-sm focus:border-health-green outline-none font-bold text-sm tracking-tight"
          />
        </div>
        <div className="flex gap-4 w-full md:w-auto">
            <StyledDropdown 
                ...
                options={['All Categories', 'Women Health', 'Mental Health', 'Food & Lifestyle', 'Heart Care']} 
                ...
                <form className="p-8 space-y-6" onSubmit={handleUpdate}>
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2 col-span-2">
                      ...
                      <input required value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full h-12 px-4 bg-slate-50 border border-black/5 rounded-xl outline-none focus:border-health-green text-sm font-bold shadow-inner" placeholder="e.g. Understanding PCOS" />
                    </div>
                    ...
                    <div className="col-span-2 sm:col-span-1">
                        <FormDropdown 
                            label="Category" 
                            value={formData.cat} 
                            options={['Women Health', 'Mental Health', 'Heart Care', 'Food & Lifestyle']} 
                    ...
                    <div className="col-span-2">
                      ...
                      <div onClick={() => fileInputRef.current.click()} className={`w-full border-2 border-dashed rounded-[2rem] p-10 flex flex-col items-center justify-center gap-4 transition-all cursor-pointer group ${selectedFile ? 'border-health-green bg-health-green/5' : 'border-slate-200 hover:border-health-green hover:bg-health-green/5'}`}>
                        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all group-hover:rotate-12 ${selectedFile ? 'bg-health-green text-white shadow-lg' : 'bg-slate-100 text-slate-400 group-hover:text-health-green'}`}>
                          {selectedFile ? <CheckCircle size={28} /> : <UploadCloud size={28} />}
                        </div>
                        <div className="text-center">
                          <p className="text-sm font-black text-slate-700">{selectedFile ? selectedFile.name : 'Click to upload or drag & drop module'}</p>
                          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mt-2 leading-none">MP4 or PDF (max. 500MB)</p>
                        </div>
                      </div>
                    </div>
                    <div className="space-y-2 col-span-2">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Description Summary</label>
                      <textarea value={formData.desc} onChange={e => setFormData({...formData, desc: e.target.value})} className="w-full h-32 p-4 bg-slate-50 border border-black/5 rounded-xl outline-none focus:border-health-green text-sm font-bold resize-none shadow-inner" placeholder="Provide a brief summary for the sisters..."></textarea>
                    </div>
                  </div>
                  <div className="pt-6 border-t border-black/5 flex justify-end gap-3 mt-4">
                    ...
                    <button type="submit" className="px-8 py-2 bg-slate-900 text-white font-black text-[9px] uppercase tracking-[0.2em] rounded-xl hover:scale-105 active:scale-95 transition-all shadow-xl shadow-slate-900/20">{editItem ? 'Update Library' : 'Commit Asset'}</button>
                  </div>
                </form>
              </div>
           </div>
        ...
        <div className="space-y-8">
           <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
                  <Clock size={12} /> {viewItem?.date}  <span className="mx-1 opacity-30">|</span> <span className="text-health-green font-black">{viewItem?.status}</span>
              </div>
           </div>
