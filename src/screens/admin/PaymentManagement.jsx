...
const StyledDropdown = ({ label, value, options, onChange, icon: Icon }) => {
    ...
    return (
      <div className="relative flex-1 w-full" ref={dropdownRef}>
        <button onClick={() => setIsOpen(!isOpen)} className={`h-10 px-4 rounded-2xl border transition-all flex items-center gap-3 font-black text-[9px] uppercase tracking-widest w-full justify-between ${isOpen ? 'bg-white border-health-green ring-4 ring-health-green/5' : 'bg-white border-black/5 hover:border-slate-200'}`}>
          <div className="flex items-center gap-3 overflow-hidden">
              <div className={`p-1 rounded-lg flex-shrink-0 ${isOpen ? 'bg-health-green text-white' : 'bg-slate-50 text-slate-400'}`}><Icon size={14} /></div>
        ...
        {isOpen && (
          <div className="absolute top-14 left-0 w-full bg-white rounded-2xl shadow-2xl border border-black/5 p-2 z-[60] animate-in zoom-in-95 duration-200">
             ...
             {options.map(opt => (
                 <button key={opt} onClick={() => { onChange(opt); setIsOpen(false); }} className={`w-full text-left px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all ${value === opt ? 'bg-health-green/10 text-health-green' : 'text-slate-500 hover:bg-slate-50'}`}>{opt}</button>
             ))}
          </div>
        )}
      </div>
  ...
  const handleDownloadInvoice = async (txn) => {
      ...
      try {
          ...
          setTimeout(() => {
              ...
              doc.setTextColor(45, 21, 32); // #2D1520
              doc.setFontSize(14);
              ...
              autoTable(doc, {
                  ...
                  headStyles: { fillColor: [45, 21, 32] }, // #2D1520 header
                  theme: 'grid'
              });
    ...
    <div className="space-y-10 font-sans pb-20 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
           ...
           <p className="text-sm text-slate-500 font-medium">Tracking subscriptions, payouts, and <span className="text-health-green font-black">GST compliance.</span></p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: 'Net Core Revenue', val: '₹4,82,400', icon: DollarSign, color: 'text-health-green', bg: 'bg-emerald-50/50' },
          ...
          <div key={i} className={`p-5 rounded-[2rem] border border-black/5 shadow-sm relative overflow-hidden group hover:shadow-md transition-all ${stat.bg}`}>
            <div className="relative z-10">
                ...
                <div className="flex items-end gap-2">
                    ...
                    <div className="flex items-center text-[9px] font-black text-health-green uppercase tracking-widest mb-1.5"><TrendingUp size={12} className="mr-1"/> 12%</div>
                </div>
            </div>
      ...
      <div className="bg-white rounded-[3rem] border border-black/5 shadow-sm overflow-hidden animate-in slide-in-from-bottom-6 duration-500">
        <div className="p-6 border-b border-black/5 flex flex-col lg:flex-row lg:items-center justify-between gap-6 bg-slate-50/20">
           <div className="flex-1 relative w-full lg:max-w-sm lg:ml-2">
              ...
              <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Find Txn ID or Customer..." className="w-full h-12 pl-12 pr-6 bg-white border border-black/5 rounded-2xl outline-none focus:border-health-green transition-all text-xs font-bold shadow-sm" />
           </div>
                 ...
                 <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-4 py-4">
                       <span className={`inline-flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest ${
                          p.status === 'Success' ? 'text-health-green' : p.status === 'Pending' ? 'text-orange-500' : 'text-red-500'
                       }`}>
                          <div className={`w-1.5 h-1.5 rounded-full ${p.status==='Success'?'bg-health-green shadow-[0_0_8px_rgba(34,197,94,0.6)]':p.status==='Pending'?'bg-orange-500 animate-pulse':'bg-red-500'}`}></div>
                          {p.status}
                       </span>
                    </td>
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
             <div className="p-6 bg-[#FFF7F8] border border-[#F5E6EA] rounded-[2rem] flex flex-col sm:flex-row items-start sm:items-center justify-between relative overflow-hidden gap-4">
                <div className="relative z-10 space-y-1">
                   ...
                   <h3 className="text-3xl font-black text-[#2D1520] tracking-tighter leading-none">{viewPay?.amount}</h3>
             ...
             <div className="space-y-3">
                 <div className="flex items-center justify-between px-2">
                    ...
                    <span className="text-[9px] font-black text-[#2D1520] uppercase tracking-widest">{viewPay?.gst}</span>
                 </div>
                 <div className="h-px bg-[#F5E6EA] w-full"></div>
                 <div className="flex items-center justify-between px-2 pt-1">
                    <span className="text-[9px] font-black text-[#2D1520] uppercase tracking-widest">Total Settled</span>
