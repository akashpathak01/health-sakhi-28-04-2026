...
const PeriodTrackerScreen = () => {
  ...
  return (
    <div className="min-h-screen w-full bg-[#FFF7F8] relative overflow-x-hidden font-sans text-[#2D1520]">
        ...
        <div className="space-y-8">
          ...
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-center gap-4 md:gap-5">
              <button 
                ...
                className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-white border border-rose-100 flex items-center justify-center text-[#2D1520] hover:bg-[#ff69b4] hover:text-white transition-all shadow-sm shrink-0"
              >
                <ChevronLeft size={18} />
              </button>
              <div>
                 <h1 className="text-2xl md:text-3xl font-black tracking-tight text-[#2D1520]">Period Tracker</h1>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
             <motion.div variants={item} className="bg-white/80 backdrop-blur-md p-5 md:p-6 rounded-2xl md:rounded-3xl border border-white shadow-sm flex items-center gap-4 md:gap-5">
                ...
                <div>
                   ...
                   <h3 className="text-lg md:text-xl font-black text-[#2D1520]">{cycleDay}</h3>
                </div>
             </motion.div>
             ...
             <motion.div variants={item} className="bg-white/80 backdrop-blur-md p-5 md:p-6 rounded-2xl md:rounded-3xl border border-white shadow-sm flex items-center gap-4 md:gap-5">
                ...
                <div>
                   ...
                   <h3 className="text-lg md:text-xl font-black text-[#2D1520]">{nextPeriodIn} Days</h3>
                </div>
             </motion.div>
             ...
             <motion.div variants={item} className="bg-[#2D1520] p-5 md:p-6 rounded-2xl md:rounded-3xl shadow-lg flex items-center gap-4 md:gap-5 text-white">
          ...
          <motion.div variants={item} className="bg-white/90 backdrop-blur-xl rounded-[2rem] md:rounded-[3rem] p-5 md:p-10 border border-white shadow-xl shadow-rose-100/20">
             <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 md:mb-10">
                <div>
                   <h3 className="text-xl md:text-2xl font-black text-[#2D1520]">April 2024</h3>
                 ...
                 return (
                   <motion.button 
                     ...
                     className={`h-11 sm:h-14 md:h-20 rounded-xl md:rounded-3xl flex flex-col items-center justify-center transition-all relative ${
                       isToday ? 'bg-[#2D1520] text-white shadow-2xl z-10' : 
                       isPeriod ? 'bg-[#ff69b4]/10 text-[#ff69b4] border border-[#ff69b4]/20' : 
                       isFertile ? 'bg-[#8b5cf6]/10 text-[#8b5cf6] border border-[#8b5cf6]/20' :
                       'bg-white/40 hover:bg-white text-[#2D1520] border border-transparent hover:border-rose-50'
                     }`}
                   >
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsModalOpen(false)} className="absolute inset-0 bg-[#2D1520]/40 backdrop-blur-sm" />
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 10 }} className="w-full max-w-md bg-white rounded-[3rem] p-10 relative z-10 shadow-2xl overflow-hidden">
              ...
              <button onClick={() => setIsModalOpen(false)} className="absolute top-8 right-8 text-[#C4A0AC] hover:text-[#2D1520] transition-colors"><X size={20} /></button>
              ...
              <div className="relative z-10 space-y-8">
                <div className="text-center space-y-2">
                  ...
                  <h3 className="text-2xl font-black text-[#2D1520]">Log Cycle 🌸</h3>
      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 50 }} className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[110] bg-[#2D1520] text-white px-8 py-5 rounded-[2rem] shadow-2xl flex items-center gap-4 border border-white/10">
