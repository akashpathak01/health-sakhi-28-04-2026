        ...
        <div className="space-y-10">
          ...
          <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
            <div className="flex items-center gap-4 md:gap-5">
              ...
              <div>
                 <h1 className="text-2xl md:text-3xl font-black tracking-tight text-[#2D1520]">Your Rewards</h1>
                 <p className="text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] text-[#C4A0AC]">Celebrate every small win</p>
              </div>
            </div>
            
            <div className="bg-[#2D1520] text-white px-6 md:px-8 py-3 md:py-4 rounded-2xl flex items-center gap-4 shadow-xl self-start sm:self-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
             ...
             <motion.div variants={item} className="bg-[#2D1520] rounded-3xl md:rounded-[2rem] p-6 md:p-8 text-white relative overflow-hidden sm:col-span-2 lg:col-span-1">
                ...
                {badges.map((badge) => {
                  ...
                  return (
                    ...
                    >
                       ...
                       <h4 className={`text-sm font-black mb-1 ${isEarned ? 'text-[#2D1520]' : 'text-[#9A8A8E]'}`}>{badge.name}</h4>
          ...
          <motion.section variants={item} className="grid grid-cols-1 lg:grid-cols-2 gap-6">
             <div className="bg-white/50 backdrop-blur-md rounded-[2rem] p-8 border border-white shadow-sm flex flex-col gap-6">
                <div className="flex items-center gap-3">
                   ...
                   <h3 className="text-[10px] font-black uppercase tracking-widest text-[#2D1520]">Recent Activity</h3>
                </div>
                <div className="space-y-4">
                   {activities.map((act) => (
                     <div key={act.id} className="flex items-center justify-between p-4 rounded-2xl bg-white/40 border border-rose-50/50">
                        <div className="space-y-1">
                           <p className="text-[11px] font-bold text-[#2D1520]">{act.text}</p>
                           <p className="text-[8px] font-black uppercase tracking-widest text-[#C4A0AC]">{act.time}</p>
                        </div>
                        <span className={`text-xs font-black ${act.amount.startsWith('+') ? 'text-[#ff69b4]' : 'text-[#2D1520]'}`}>
                           {act.amount} 🪙
                        </span>
                     </div>
                   ))}
             ...
             <div className="bg-gradient-to-br from-[#2D1520] to-[#1a0c13] rounded-[2rem] p-8 text-white flex flex-col justify-between relative overflow-hidden group">
                ...
                <button className="relative z-10 w-full py-4 bg-white text-[#2D1520] rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-[#ff69b4] hover:text-white transition-all shadow-xl">
                   Share Invite Link
                </button>
             </div>
          </motion.section>
          ...
          <motion.section variants={item} className="bg-white/80 backdrop-blur-xl rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-10 border border-white shadow-xl shadow-rose-100/30 relative overflow-hidden">
             <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between relative z-10 mb-8 md:mb-10 gap-4">
                <div className="flex items-center gap-4 md:gap-5">
                   <div className="w-10 h-10 md:w-12 md:h-12 bg-[#2D1520] rounded-xl flex items-center justify-center text-white shadow-lg shrink-0"><Gift size={20} md:size={24} /></div>
