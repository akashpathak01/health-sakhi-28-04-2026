      ...
      <div className="space-y-12 pt-8">
        <motion.section variants={item} className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div className="space-y-1">
            <h2 className="text-4xl font-black tracking-tight" style={{ color: '#2D1520' }}>Wealth Sakhi 💰</h2>
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-8 space-y-12">
            {/* Overview Card */}
            <motion.section variants={item} className="bg-[#2D1520] rounded-[3rem] p-10 md:p-14 text-white relative overflow-hidden">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                 {goals.map((goal, i) => (
                   <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-[#F5E6EA] shadow-sm hover:shadow-xl transition-all group">
                      ...
                      <h4 className="font-black text-lg mb-4" style={{ color: '#2D1520' }}>{goal.title}</h4>
                      <div className="mb-4">
                         <div className="flex justify-between text-[10px] font-black uppercase mb-2">
                            <span style={{ color: '#C4A0AC' }}>Target</span>
                            <span style={{ color: '#2D1520' }}>{goal.target}</span>
                         </div>
          ...
          <div className="lg:col-span-4 space-y-10">
            ...
            <motion.section variants={item} className="bg-white rounded-[3rem] p-8 border border-[#F5E6EA] shadow-sm">
               ...
               <div className="space-y-4">
                  {[
                    ...
                    { label: 'Cash', value: 10, color: '#2D1520' }
                  ].map((asset, i) => (
                    <div key={i}>
                       <div className="flex justify-between text-[10px] font-black uppercase mb-1">
                          <span style={{ color: '#6B5E63' }}>{asset.label}</span>
                          <span style={{ color: '#2D1520' }}>{asset.value}%</span>
                       </div>
                       <div className="h-1.5 w-full bg-rose-50 rounded-full overflow-hidden">
            ...
            <motion.section variants={item} className="rounded-[3rem] p-8 bg-gradient-to-br from-[#FFF9EE] to-[#FEF9E3] border border-[#F5E6EA]">
               ...
               <h3 className="text-xl font-black mb-3" style={{ color: '#2D1520' }}>Financial Wisdom</h3>
               ...
               <div className="space-y-6">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between">
                       <div className="flex flex-col">
                          <span className="text-[11px] font-black text-[#2D1520]">{item.label}</span>
