...
const AffirmationScreen = () => {
  ...
  const nextAffirmation = () => {
    ...
    setIsFlipping(true);
    setTimeout(() => {
      const list = isCoupleMode ? affirmations['Couples'] : affirmations[activeTab];
    ...
    <div className="min-h-screen pb-20 px-4 md:px-10 bg-gradient-to-br from-[#FAF7F8] via-white to-[#FDF0F3] relative overflow-hidden font-sans">
      ...
      <div className="pt-10 text-center relative z-10">
        <motion.h1 
          ...
          className="text-4xl md:text-6xl font-black tracking-tighter text-[#2D1520] drop-shadow-[0_0_25px_rgba(209,123,136,0.3)]"
        >
          ...
          {tabs.map((tab) => (
            <button
              ...
              className={`flex items-center gap-3 px-6 py-3.5 rounded-[1.8rem] transition-all duration-500 whitespace-nowrap ${activeTab === tab.id
                ? 'bg-gradient-to-r from-[#2D1520] to-[#452433] text-white shadow-xl scale-105'
      ...
      <div className="mt-16 max-w-7xl mx-auto relative z-10 px-4">
        <div className="relative w-full aspect-[16/10] md:aspect-[16/8] perspective-2000">
          ...
          {/* Book Wrapper */}
          <div className="absolute inset-0 bg-[#2D1520] rounded-[3rem] shadow-[0_30px_100px_rgba(45,21,32,0.2)] border-r-[12px] border-[#3D2530] flex">
                         ...
                         {isCoupleMode ? (
                           <div className="space-y-8 max-w-sm">
                             <div className="p-4 rounded-2xl bg-rose-50/30">
                               ...
                               <p className="text-lg md:text-xl font-black text-[#2D1520] italic">"{getCurrentAffirmation().left}"</p>
                             </div>
                             ...
                             <div className="p-4 rounded-2xl bg-[#F5F1FE]/30">
                               ...
                               <p className="text-lg md:text-xl font-black text-[#2D1520] italic">"{getCurrentAffirmation().right}"</p>
                             </div>
                           </div>
                         ) : (
                           <p className="text-2xl md:text-3xl lg:text-4xl font-black text-[#2D1520] leading-tight tracking-tight italic px-6 font-serif">
          ...
          {/* Nav Controls */}
          <button onClick={prevAffirmation} className="absolute left-[-25px] top-1/2 -translate-y-1/2 w-14 h-14 bg-white rounded-full shadow-2xl flex items-center justify-center text-[#2D1520] hover:scale-110 active:scale-95 transition-all z-40 border border-rose-50">
            <ChevronLeft size={24} />
          </button>
          <button onClick={nextAffirmation} className="absolute right-[-25px] top-1/2 -translate-y-1/2 w-14 h-14 bg-white rounded-full shadow-2xl flex items-center justify-center text-[#2D1520] hover:scale-110 active:scale-95 transition-all z-40 border border-rose-50">
        ...
        <div className="mt-12 flex justify-center gap-8">
           {[
             ...
             { icon: Maximize2, label: 'Full Read', onClick: () => setIsFullScreen(true), color: '#2D1520' },
           ].map((action, i) => (
             <button key={i} onClick={action.onClick} className="flex flex-col items-center gap-2.5 group">
                <motion.div 
                  whileHover={{ y: -5 }}
                  className="w-16 h-16 rounded-[1.8rem] bg-white shadow-lg flex items-center justify-center border border-rose-50 group-hover:bg-[#2D1520] group-hover:text-white transition-all duration-300"
                >
                   <action.icon size={22} />
                </motion.div>
                <span className="text-[9px] font-black uppercase tracking-widest text-[#A98DA4] group-hover:text-[#2D1520] transition-colors">{action.label}</span>
             </button>
           ))}
        </div>
      </div>
      ...
      <AnimatePresence>
         {isMeditationMode && (
           ...
           >
              <div className="bg-[#2D1520] p-10 md:p-16 rounded-[3.5rem] text-center relative overflow-hidden">
      ...
      <div className="mt-24 max-w-6xl mx-auto px-4 pb-12">
         <div className="relative overflow-hidden bg-[#2D1520] rounded-[3rem] p-10 md:p-20 shadow-[0_40px_80px_-20px_rgba(45,21,32,0.3)]">
            {/* Background Accents */}
