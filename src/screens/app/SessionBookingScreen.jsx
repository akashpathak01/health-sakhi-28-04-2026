...
const SuccessScreen = ({ booking, onReset }) => (
  <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
    ...
    <h2 className="text-3xl font-black text-[#2D1520] mb-2">See you soon! 🎉</h2>
    ...
    <div className="bg-white rounded-[2.5rem] p-8 border border-rose-50 shadow-sm text-left space-y-4 mb-8">
      {[
        ...
        <div key={i} className="flex justify-between items-center py-2 border-b border-rose-50 last:border-0">
          ...
          <span className="text-xs font-bold text-[#2D1520]">{r.value}</span>
        </div>
      ))}
    </div>
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 space-y-12">
      ...
      <section className="px-2 lg:px-0">
        <div className="rounded-[3rem] p-10 flex flex-col md:flex-row items-center gap-10 border shadow-sm" style={{ background: '#FFF7F8', borderColor: '#F5E6EA' }}>
          <div className="flex-1 space-y-6">
            ...
            <h1 className="text-4xl font-bold tracking-tight leading-tight text-[#2D1520]">
          ...
          <div className="w-56 h-56 bg-white rounded-[2.5rem] shadow-sm flex items-center justify-center p-8 border" style={{ borderColor: '#F5E6EA' }}>
            <div className="text-center">
              ...
              <p className="text-[10px] font-extrabold uppercase tracking-widest text-[#2D1520]">Video Consultation</p>
        ...
        {mySessions.length > 0 && (
          <motion.section initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6 px-2 lg:px-0">
            <div className="flex items-center justify-between">
               <h2 className="text-xl font-black text-[#2D1520] tracking-tight">Your Upcoming Sessions</h2>
                 ...
                 <motion.div key={session.id} layout
                   ...
                   <div className="flex items-start gap-4 mb-6 relative z-10">
                      ...
                      <div>
                         ...
                         <h3 className="text-sm font-black text-[#2D1520] leading-tight">{session.advisor}</h3>
                         <p className="text-[10px] font-bold text-[#C4A0AC]">{session.topic}</p>
                      </div>
                   </div>
                   <div className="space-y-3 mb-6 relative z-10">
                      <div className="flex items-center gap-3 text-xs font-bold text-[#2D1520]">
      ...
      <section className="space-y-8 px-2 lg:px-0">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-[#2D1520]">Our Wellness Advisors</h2>
                ...
                <div className="flex-1 space-y-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="text-xl font-bold text-[#2D1520]">{adv.name}</h3>
                  ...
                  <div className="flex items-center gap-6 pt-2">
                    <div className="flex items-center gap-1.5 font-bold text-xs text-[#2D1520]">
                    ...
                    <motion.button whileTap={{ scale: 0.96 }}
                      ...
                      style={{ background: selectedAdvisor?.id === adv.id ? '#2D1520' : '#ff69b4' }}>
          ...
          {!selectedAdvisor ? (
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <div className="max-w-md mx-auto space-y-3">
                <h2 className="text-2xl font-bold text-[#2D1520]">Select your slot.</h2>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <h3 className="text-lg font-black text-[#2D1520]">{selectedAdvisor.name}</h3>
                  </div>
                </div>
                ...
                <div className="flex gap-4">
                  {durations.map((d, i) => (
                    <motion.button key={i} whileTap={{ scale: 0.96 }}
                      ...
                      <p className={`text-lg font-black ${selectedDuration === i ? 'text-[#ff69b4]' : 'text-[#2D1520]'}`}>{d.label}</p>
                ...
                <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
                  {days.map((d) => (
                    <motion.button key={d.idx} whileTap={{ scale: 0.95 }}
                      ...
                      className={`flex-shrink-0 px-5 py-3 rounded-2xl border-2 transition-all text-center min-w-[80px] ${selectedDay === d.idx ? 'bg-[#2D1520] border-[#2D1520] text-white' : 'border-[#F5E6EA] hover:border-rose-200'}`}>
                      <p className={`text-[10px] font-black uppercase tracking-wide ${selectedDay === d.idx ? 'text-white/60' : 'text-[#C4A0AC]'}`}>{d.label}</p>
                      <p className={`text-sm font-black ${selectedDay === d.idx ? 'text-white' : 'text-[#2D1520]'}`}>{d.date}</p>
                    </motion.button>
                  ))}
                </div>
              </div>
                ...
                <div className="grid grid-cols-3 md:grid-cols-5 gap-3">
                  {(selectedAdvisor.slots[selectedDay] || []).map((slot) => (
                    <motion.button key={slot} whileTap={{ scale: 0.95 }}
                      ...
                      className={`py-3 rounded-2xl border-2 text-xs font-black transition-all ${selectedSlot === slot ? 'bg-[#ff69b4] border-[#ff69b4] text-white shadow-lg shadow-rose-200' : 'border-[#F5E6EA] text-[#2D1520] hover:border-[#ff69b4]'}`}>
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="relative">
                        ...
                        <input type="text" placeholder="Your Name"
                          ...
                          className="w-full pl-10 pr-4 py-4 rounded-2xl border-2 border-[#F5E6EA] focus:border-[#ff69b4] outline-none text-sm font-bold text-[#2D1520] bg-white transition-all" />
                      </div>
                      <div className="relative">
                        <MessageSquare size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-[#C4A0AC]" />
                        <select value={topic} onChange={e => setTopic(e.target.value)}
                          className="w-full pl-10 pr-4 py-4 rounded-2xl border-2 border-[#F5E6EA] focus:border-[#ff69b4] outline-none text-sm font-bold text-[#2D1520] bg-white transition-all appearance-none">
                    ...
                    <div className="bg-rose-50/50 rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-4">
                      <div className="space-y-1 text-sm">
                        <p className="font-black text-[#2D1520]">{selectedAdvisor.name} • {days[selectedDay].label}, {days[selectedDay].date}</p>
