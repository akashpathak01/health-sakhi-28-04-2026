...
const MeditationScreen = () => {
  ...
  const filteredSessions = useMemo(() => {
    if (activeCategory === 'all') return MED_SESSIONS;
    return MED_SESSIONS.filter(s => s.category === activeCategory);
  }, [activeCategory]);
    ...
    <div className="w-full px-4 py-8 min-h-screen bg-[#FDF7F8] font-sans">
      ...
      <header className="flex items-center justify-between mb-10">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 bg-[#2D1520] rounded-2xl flex items-center justify-center text-white shadow-xl shadow-rose-950/20">
             <Sparkles size={24} />
          </div>
          <div>
            <h1 className="text-2xl font-black text-[#2D1520] leading-none mb-1">Health Sakhi</h1>
        ...
        <div className="bg-white border border-[#F5E6EA] py-2 px-6 rounded-2xl flex items-center gap-3">
           ...
           <span className="text-lg font-black text-[#2D1520]">{totalMinutesToday}m Today</span>
        </div>
      </header>
                  ...
                  {CATEGORIES.map((cat) => (
                    <button
                      ...
                      className={`flex items-center gap-2 px-6 py-4 rounded-full text-[11px] font-black uppercase tracking-widest transition-all ${
                        activeCategory === cat.id ? 'bg-[#2D1520] text-white shadow-xl translate-y-[-2px]' : 'bg-white text-[#C4A0AC] border border-[#F5E6EA] hover:bg-rose-50'
                      }`}
                    >
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                  {filteredSessions.map((session) => (
                    ...
                    >
                       ...
                       <div className="p-9">
                          <h4 className="text-2xl font-black text-[#2D1520] mb-1 group-hover:text-rose-500 transition-colors">{session.title}</h4>
            ...
            <aside className="lg:col-span-4 space-y-8">
               <section className="bg-white rounded-[40px] p-10 border border-[#F5E6EA] shadow-xl">
                  ...
                  <div className="p-6 bg-rose-50 rounded-3xl mb-8 flex items-center gap-4">
                     ...
                     <p className="text-xs font-bold text-[#2D1520]/80">AI Audio Guide is currently active for playback.</p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                     <div className="p-5 bg-[#FDF7F8] rounded-[24px] text-center border border-rose-50">
                        ...
                        <p className="text-2xl font-black text-[#2D1520]">12d</p>
                        <p className="text-[9px] font-black uppercase text-[#C4A0AC]">Streak</p>
                     </div>
                     <div className="p-5 bg-[#FDF7F8] rounded-[24px] text-center border border-rose-50">
                        <Heart size={24} className="text-indigo-400 mx-auto mb-2" />
                        <p className="text-2xl font-black text-[#2D1520]">Lvl 4</p>
                        <p className="text-[9px] font-black uppercase text-[#C4A0AC]">Zen</p>
                     </div>
                  </div>
               </section>
               <div className="p-9 bg-white rounded-[40px] text-[#2D1520] border border-[#F5E6EA] shadow-xl relative overflow-hidden group">
               ...
               <div className="lg:w-[40%] p-10 md:p-14 flex flex-col justify-between bg-white relative">
                  ...
                  <div className="space-y-12">
                    <div>
                       ...
                       <h2 className="text-4xl font-black text-[#2D1520] tracking-tight leading-none mb-6">{selectedSession.title}</h2>
                    ...
                    <div className="space-y-4">
                       ...
                       <div className="flex items-baseline gap-2">
                          <span className="text-6xl font-black text-[#2D1520] tabular-nums">{formatTime(timeRemaining)}</span>
                       </div>
                    </div>
                  </div>
                  ...
                  <footer className="space-y-14 mt-12">
                     <div className="flex items-center justify-center gap-10">
                        <button onClick={() => skipTime(-10)} className="text-[#C4A0AC] hover:text-[#2D1520] transition-all"><Rewind size={36} /></button>
                        <button onClick={() => setIsPlaying(!isPlaying)} className="w-24 h-24 bg-[#2D1520] text-white rounded-full flex items-center justify-center shadow-[0_20px_50px_-10px_rgba(45,21,32,0.4)] hover:scale-105 active:scale-95 transition-all">
                           {isPlaying ? <Pause size={44} fill="currentColor" /> : <Play size={44} fill="currentColor" className="ml-2" />}
                        </button>
                        <button onClick={() => skipTime(10)} className="text-[#C4A0AC] hover:text-[#2D1520] transition-all"><Forward size={36} /></button>
                     </div>
                     ...
                     <div className="bg-[#FDF7F8] p-6 rounded-[32px] border border-rose-50 flex items-center justify-between shadow-inner">
                        <div className="flex items-center gap-4">
                           ...
                           <div>
                              ...
                              <p className="text-[11px] font-black text-[#2D1520]">Dynamic AI Voice</p>
                           </div>
                        </div>
