...
const AISakhiScreen = () => {
  ...
  return (
    <div className="h-[calc(100vh-140px)] md:h-[calc(100vh-160px)] w-full flex flex-col relative font-sans text-[#2D1520]">
      ...
      <div className="px-4 md:px-8 py-4 md:py-5 flex items-center justify-between flex-shrink-0 relative z-30">
        <div className="flex items-center gap-3 md:gap-5">
          <button 
            ...
            className="w-9 h-9 md:w-10 md:h-10 rounded-full border border-rose-100 flex items-center justify-center text-[#2D1520] bg-white shadow-sm hover:bg-rose-50 transition-all shrink-0"
          >
            <ChevronLeft size={18} />
          </button>
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div>
              <h2 className="text-lg md:text-xl font-black tracking-tight text-[#2D1520]">AI Sakhi</h2>
              ...
              >
                ...
                <div className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'} max-w-[85%] md:max-w-[70%]`}>
                  <div className={`px-4 md:px-6 py-3 md:py-4 text-[13px] md:text-sm font-semibold leading-relaxed shadow-xl ${
                    msg.role === 'user' 
                    ? 'bg-[#2D1520] text-white rounded-2xl md:rounded-[2rem] rounded-br-none' 
                    : 'bg-white text-[#2D1520] rounded-2xl md:rounded-[2rem] rounded-bl-none border border-rose-50'
                  }`}>
                    {msg.text}
                  </div>
                  {msg.suggestions && (
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
            {quickReplies.map(r => (
              <button 
                ...
                className="px-6 py-3 bg-white border border-rose-100 rounded-xl text-[9px] font-black uppercase tracking-widest text-[#2D1520] whitespace-nowrap shadow-sm hover:border-[#ff69b4] transition-all"
              >
                {r}
              </button>
            ))}
