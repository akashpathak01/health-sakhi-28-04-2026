    ...
    <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
      ...
      <div className="space-y-4 mb-8">
        {[
          ...
          <div key={i} className="flex justify-between items-start py-3 border-b border-rose-50">
            ...
            <span className="text-xs font-bold text-[#2D1520] text-right max-w-[55%]">{d.value}</span>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-2 gap-4">
        ...
        <button onClick={onBook}
          className="py-4 bg-[#2D1520] rounded-2xl text-[10px] font-black uppercase tracking-widest text-white hover:brightness-110 transition-all shadow-lg">
          Book Session
        </button>
      </div>
    </motion.div>
        ...
        <div className="p-8 bg-white border-t border-rose-50">
          <div className="flex gap-4">
            <input 
              ...
              className="flex-1 px-6 py-4 rounded-2xl bg-rose-50/50 border-2 border-transparent focus:border-[#ff69b4] focus:bg-white outline-none text-sm font-medium text-[#2D1520] transition-all"
            />
            <button 
              onClick={handleSend}
              disabled={!input.trim()}
...
const EventModal = ({ event, onClose, onRegister }) => (
  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }}
      ...
      <h3 className="text-2xl font-black text-[#2D1520] mb-4 leading-tight">{event.title}</h3>
      <p className="text-sm text-[#9A8A8E] leading-relaxed mb-6">{event.desc}</p>
      <div className="space-y-3 mb-8">
        <div className="flex items-center gap-3">
          <Clock size={14} className="text-[#ff69b4]" />
          <span className="text-xs font-bold text-[#2D1520]">{event.time}</span>
        </div>
        <div className="flex items-center gap-3">
          <MapPin size={14} className="text-[#ff69b4]" />
          <span className="text-xs font-bold text-[#2D1520]">Online • Zoom Link Sent After Registration</span>
        </div>
      </div>
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="w-full pb-20 px-4 pt-10">
      ...
      <motion.section variants={item} className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div className="space-y-1">
          <h2 className="text-4xl font-black tracking-tight text-[#2D1520]">Relationship Sakhi 💞</h2>
          <span className="text-[11px] font-black uppercase tracking-[0.3em] text-[#C4A0AC]">Nurturing Beautiful Bonds</span>
        </div>
        <motion.button
          whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }}
          onClick={() => navigate('/app/sessions')}
          className="px-8 py-4 bg-[#2D1520] text-white rounded-[2rem] text-xs font-black uppercase tracking-widest shadow-xl hover:bg-black transition-all">
          Book Counseling
        </motion.button>
      </motion.section>
      ...
      <main className="grid grid-cols-1 lg:grid-cols-12 gap-10">
              ...
              <div>
                ...
                <div className="flex gap-4 flex-wrap">
                  ...
                  <motion.button
                    ...
                    className="px-6 py-3 bg-white border border-[#F5E6EA] text-[#2D1520] rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-50 transition-all">
                    View Profile
                  </motion.button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {learningTracks.map((track, i) => (
                <motion.div key={i} whileHover={{ y: -6 }}
                  ...
                  <h4 className="text-lg font-black mb-2 text-[#2D1520]">{track.title}</h4>
        ...
        <div className="lg:col-span-4 space-y-10">
          ...
          {/* Community Card */}
          <motion.div variants={item}
            className="bg-[#2D1520] rounded-[3rem] p-10 text-white shadow-2xl relative overflow-hidden group cursor-pointer"
              ...
              {upcomingEvents.map((event, i) => (
                <motion.div key={i} whileHover={{ x: 4 }}
                  ...
                  <div className="flex-1">
                    <h4 className="text-[13px] font-black text-[#2D1520] group-hover:text-[#ff69b4] transition-colors leading-tight">
                      {event.title}
                    </h4>
          ...
          <motion.div variants={item}
            ...
            <p className="text-sm font-bold text-[#2D1520] leading-relaxed">
